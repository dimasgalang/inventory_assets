...
class FabricQRController extends Controller
{
    public function index(Request $request)
    {
        ...
        // }
        $fabricqrs = FabricsQR::all();
        return view('fabricqr.index', compact('fabricqrs'));
    }
    ...
    {
        $fabricqrs = FabricsQR::where('qr_code', '=', null)->get();
        foreach ($fabricqrs as $fabricqr) {
            $qr_data = $fabricqr->random_qr_code;

            $fileImageName = $qr_data . '.jpg';

            // Create image (base64) from some text
            $string = $fabricqr->random_qr_code;
            $width  = 200;
            $height = 200;
            $font   = 6;
            $im = @imagecreate(
            ...
            $file = 'public/fabricsqr/text/' . $qr_data . '.jpg';
            // Move to folder
            // file_put_contents($file, $image_base64);
            Storage::put('public/fabricsqr/text/' . $fileImageName, $image_base64);

            $qr = FacadesQrCode::format('png')->margin(12)->merge(storage_path('app/public/fabricsqr/text/') . $qr_data . '.jpg', 1, true)->size(200)->generate($qr_data);

            // $qr = FacadesQrCode::format('png')->generate($qr_data);
            // $qrImageName = $qr_data . '.png';

            Storage::put('public/fabricsqr/' . $fileImageName, $qr);

            $updatefabricqr = FabricsQR::findOrFail($fabricqr->id);
    ...
    public function importFabric(Request $request)
    {
        ...
        $import = Excel::import(new FabricsQRImport(), storage_path('app/public/excel/' . $nama_file));
        Storage::delete($path);
    ...
    public function generatePDF(Request $request)
    {
        ...
        if ($request->exporttype == "all") {
            $qrcodes = FabricsQR::all();
            $document = "All Fabrics QR Codes Sticker.pdf";
        } else {
            $qrcodes = FabricsQR::whereBetween('assets_number', [$request->from_assets_number, $request->to_assets_number])->get();
            $document = "Fabrics QR Codes Sticker (" . $request->from_assets_number . " - " . $request->to_assets_number . ").pdf";
        }
        $data = ['title' => $document];
