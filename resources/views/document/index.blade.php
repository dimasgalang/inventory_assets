            ...
            <div class="container-fluid">
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3 d-sm-flex align-items-center justify-content-between mb-4">
                        <h6 class="m-0 font-weight-bold text-primary">Document Data</h6>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <a class="btn btn-primary btn-circle btn-sm btn-show-pdf" data-show-link="{{ $document->original_name }}" data-show-title="{{ $document->document_name }}" data-toggle="modal" data-target="#pdfModal">
                                                <i class="fas fa-file-pdf"></i>
                                            </a>
                                            <a class="btn btn-danger btn-circle btn-sm btn-void-record" data-void-link="{{ route('document.void', ['id' => $document->id]) }}" data-void-name="{{ $document->document_name }}" data-toggle="modal" data-target="#voidModal">
                                                <i class="fas fa-ban"></i>
                                            </a>
                                            @elseif (request()->get('void') == 'true')
                                            <a class="btn btn-success btn-circle btn-sm btn-restore-record" data-restore-link="{{ route('document.restore', ['id' => $document->id]) }}" data-restore-name="{{ $document->document_name }}" data-toggle="modal" data-target="#restoreModal">
...
<script type="text/javascript">
    $('.btn-show-pdf').on('click', function () {
        $('#pdf-src').attr('src', '../../storage/document/' + $(this).data('show-link'));
        $("#pdf-title").text($(this).data('show-title'));
    });
    $('.btn-delete-record').on('click', function () {
            $('#btn-confirm').attr('href', $(this).data('delete-link'));
            $("#modal-text-record").text('Apakah anda yakin ingin menghapus Document ' + $(this).data('delete-name') + '?');
    });
    $('.btn-void-record').on('click', function () {
            $('#btn-confirm-void').attr('href', $(this).data('void-link'));
            $("#modal-text-record-void").text('Apakah anda yakin ingin menghapus Document ' + $(this).data('void-name') + '?');
    });
    $('.btn-restore-record').on('click', function () {
            $('#btn-confirm-restore').attr('href', $(this).data('restore-link'));
            $("#modal-text-record-restore").text('Apakah anda yakin ingin mengembalikan Document ' + $(this).data('restore-name') + '?');
    });
</script>
</html>