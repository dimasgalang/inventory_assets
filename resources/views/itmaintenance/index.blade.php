...
@include('layout.sidebar')
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">IT Maintenance List</h1>
                    <div>
                        <a href="{{ route('itmaintenance.fetchassetscomputer') }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                ...
                <div class="card shadow mb-4">
                    <div class="card-header py-3 d-sm-flex align-items-center justify-content-between mb-4">
                        <h6 class="m-0 font-weight-bold text-primary">IT Maintenance Data</h6>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-sm" id="dataTable" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    @foreach($itmaintenances as $itmaintenance)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $itmaintenance->month }}</td>
                                        <td>{{ $itmaintenance->year }}</td>
                                        <td>{{ $itmaintenance->assets_number }}</td>
                                        <td>{{ $itmaintenance->condition }}</td>
                                        <td>{{ $itmaintenance->checking_date }}</td>
                                        <td>{{ $itmaintenance->checker }}</td>
                                        <td>{{ $itmaintenance->remark }}</td>
                                        <td class="text-center">
                                            @if (request()->get('void') == 'false' || request()->get('void') == '')
                                            <a class="btn btn-danger btn-circle btn-sm btn-void-record" data-void-link="{{ route('itmaintenance.void', ['id' => $itmaintenance->id]) }}" data-void-name="{{ $itmaintenance->item_name }}" data-toggle="modal" data-target="#voidModal">
                                                <i class="fas fa-ban"></i>
                                            </a>
                                            @elseif (request()->get('void') == 'true')
                                            <a class="btn btn-success btn-circle btn-sm btn-restore-record" data-restore-link="{{ route('itmaintenance.restore', ['id' => $itmaintenance->id]) }}" data-restore-name="{{ $itmaintenance->item_name }}" data-toggle="modal" data-target="#restoreModal">
        ...
        <div class="modal fade" id="pdfModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl" role="itmaintenance" >
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 id="pdf-title" class="modal-title" id="exampleModalLabel">IT Maintenance Name</h5>
        ...
        <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-md" role="itmaintenance" >
                <div class="modal-content">
                    <div class="modal-header">
        ...
        <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-md" role="document" >
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 id="modal-title" class="modal-title" id="exampleModalLabel">Import IT Maintenance</h5>
                        <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">x</span>
                        </button>
                    </div>
                    <form action="{{ route('itmaintenance.import') }}" method="POST" enctype="multipart/form-data">
                        @csrf
...
<script type="text/javascript">
    $('.btn-show-pdf').on('click', function () {
        $('#pdf-src').attr('src', '../../storage/itmaintenance/' + $(this).data('show-link'));
        $("#pdf-title").text($(this).data('show-title'));
    });
    $('.btn-delete-record').on('click', function () {
            $('#btn-confirm').attr('href', $(this).data('delete-link'));
            $("#modal-text-record").text('Apakah anda yakin ingin menghapus IT Maintenance ' + $(this).data('delete-name') + '?');
    });
    $('.btn-void-record').on('click', function () {
            $('#btn-confirm-void').attr('href', $(this).data('void-link'));
            $("#modal-text-record-void").text('Apakah anda yakin ingin menghapus IT Maintenance ' + $(this).data('void-name') + '?');
    });
    $('.btn-restore-record').on('click', function () {
            $('#btn-confirm-restore').attr('href', $(this).data('restore-link'));
            $("#modal-text-record-restore").text('Apakah anda yakin ingin mengembalikan IT Maintenance ' + $(this).data('restore-name') + '?');
    });
